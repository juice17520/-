[index.html](https://github.com/user-attachments/files/24488372/index.html)
<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>旅遊規劃器 - 終極修復版</title>
...
<body class="bg-gray-50 text-slate-800">
    ...
    <header class="bg-brand text-white p-6 shadow-xl">
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <h1 class="text-2xl font-bold tracking-wider">🌿 TRAVEL PLANNER</h1>
            <div class="flex gap-2 items-end">
                ...
                <button onclick="initTrip()" class="bg-white text-brand px-4 py-1.5 rounded font-bold hover:bg-gray-100 transition shadow-md text-sm">生成天數</button>
                <button onclick="localStorage.clear(); location.reload();" class="bg-red-500 text-white px-2 py-1.5 rounded text-[10px] hover:bg-red-600 transition">重置數據</button>
            </div>
        </div>
    </header>

    <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-6 p-6">
        <div id="dayTabs" class="space-y-2">
            </div>
            ...
            <div id="mainEditor" class="hidden">
                <div class="bg-white p-6 rounded-xl shadow-sm mb-6 border-t-4 border-brand">
                    <div class="flex justify-between items-center mb-4">
                        <h2 id="currentDayTitle" class="text-xl font-bold text-gray-700">請選取日期</h2>
                        <span class="text-[10px] text-brand bg-green-50 px-2 py-1 rounded font-bold border border-green-100 italic">長按卡片刪除行程</span>
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-6 gap-3 bg-gray-50 p-4 rounded-lg border border-gray-100 shadow-inner">
                        <div class="md:col-span-1"><label class="text-[10px] font-bold text-gray-400">時間</label><input type="time" id="itemTime" class="w-full p-2 border rounded text-sm"></div>
                        <div class="md:col-span-1"><label class="text-[10px] font-bold text-gray-400">類型</label><select id="itemType" class="w-full p-2 border rounded text-sm"><option>景點</option><option>美食</option><option>交通</option><option>飯店</option></select></div>
                        <div class="md:col-span-2"><label class="text-[10px] font-bold text-gray-400">地點</label><input type="text" id="itemTitle" placeholder="要去哪？" class="w-full p-2 border rounded text-sm"></div>
                        <div class="md:col-span-2"><label class="text-[10px] font-bold text-gray-400">備註</label><div class="flex gap-2"><input type="text" id="itemNote" placeholder="選填" class="w-full p-2 border rounded text-sm"><button onclick="addItem()" class="bg-brand text-white px-4 py-2 rounded font-bold text-sm shadow-md">新增</button></div></div>
                    </div>
                </div>
...
// 自動初始化：如果完全沒資料，幫用戶建一個預設的
if (Object.keys(db).length === 0) {
    db["預設行程"] = { start: "", end: "", data: {} };
    currentTripId = "預設行程";
}
...
function loadTripData() {
    ...
    if (trip.start && trip.end) {
        initTrip(true); // 呼叫原本的生成天數邏輯，傳入 true 代表是靜默載入
    } else {
...
function updateTripSelector() {
    ...
    if (!selectorWrap) {
        ...
        selectorWrap.innerHTML = `
            <span class="text-[10px] font-bold uppercase opacity-70 shrink-0">目前計畫:</span>
            ...
            <button onclick="createNewTrip()" class="bg-white/20 text-white px-3 py-1 rounded text-xs font-bold whitespace-nowrap">+ 新計畫</button>
        `;
...
function createNewTrip() {
    const name = prompt("請輸入新計畫的名稱 (例如: 2月日本行):");
    if (!name || name.trim() === "") return;
    if (db[name]) return alert("此名稱已存在");
        ...
        while (curr <= end) {
            ...
            (function(d, b) {
                btn.onclick = function() {
                    ...
                    document.getElementById('currentDayTitle').innerText = `📅 ${d} 行程清單`;
                    renderItems();
                };
            })(dateStr, btn);
...
function addItem() {
    if (!currentActiveDate) return alert("請先在左邊選一個日期！");
    const time = document.getElementById('itemTime').value;
    const title = document.getElementById('itemTitle').value;
    if (!time || !title) return alert("時間和地點是必填的！");
...
function renderItems() {
    ...
    if (items.length === 0) {
        container.innerHTML = `<p class="text-center text-slate-300 py-20 text-sm italic tracking-widest">點擊上方「新增」規劃這天的行程</p>`;
        return;
    }
    items.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <div class="flex-1 bg-white border border-gray-100 p-4 rounded-lg shadow-sm -mt-2 event-card"
                ...
                <div class="flex justify-between items-center mb-1">
                    ...
                    <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.title)}" target="_blank" class="text-blue-400 text-[10px] font-bold">🗺️ 導航</a>
                </div>
